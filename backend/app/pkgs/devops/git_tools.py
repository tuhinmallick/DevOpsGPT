...
def pullCode(ws_path, repo_path, base_branch, feature_branch, gitConfigList):
    ...
    try:
        ...
        return False, f"mkdir failed: {str(e)}"

    try:
        shutil.move(
            f'{ws_path}/{repo_path}',
            f'{ws_path}/{repo_path}_bak{generate_uuid()}',
        )
    except Exception as e:
        print(f"pullCode move failed {str(e)}")
    ...
    if result.returncode != 0:
        print(f"git clone feature_branch failed: {result.stderr}")

        result = subprocess.run(['git', 'clone', '-b', base_branch, gitUrl, repo_path], capture_output=True, text=True, cwd=ws_path)
        if result.returncode != 0:
            print(f"git clone base_branch failed: {result.stderr}")
            # 克隆失败，说明目录已存在，尝试重置目录
            return False, f"git clone branch failed: {result.stderr}"

        result = subprocess.run(
            ['git', 'checkout', '-b', feature_branch],
            capture_output=True,
            text=True,
            cwd=f'{ws_path}/{repo_path}',
        )
    if result.returncode != 0:
        print(result.stderr)
        return False, f"git checkout branch failed: {result.stderr}"
    ...
    if result.returncode != 0:
        print(result.stderr)
        return False, f"git config pull.rebase false failed: {result.stderr}"
...
def gitResetWorkspace(wsPath, gitPath, fatureBranch, commitMsg, gitConfigList):
    ...
    if result.returncode != 0:
        print(f"git fetch origin fatureBranch false failed: {result.stderr}")
        return True, result.stderr

    result = subprocess.run(
        ['git', 'reset', '--hard', f'origin/{fatureBranch}'],
        capture_output=True,
        text=True,
        cwd=gitCwd,
    )
    if result.returncode != 0:
        ...
        return (
            False,
            f"git reset --hard origin fatureBranch false failed: {result.stderr}",
        )
